...
describe("Memes class", () => {
    it("getBestMemes method - normal conditions", () => {
    ...
    it("getBestMemes method - less than 3 memes", () => {
        const memes = new Array<Meme>(10);
        for (let i = 0; i < 2; i++)
            memes[i] = new Meme(i, 'Meme' + i.toString(), 10 * i, '/sok');
        ...
        const memesInstance = new Memes(memes);
        const bestMemes = memesInstance.getBestMemes();
        expect(bestMemes[0].price).to.equal(10);
        expect(bestMemes[1].price).to.equal(0);
        expect(bestMemes[2]).to.equal(null);
    });

    it("getBestMemes method - no memes", () => {
        const memes = new Array<Meme>(10);

        const memesInstance = new Memes(memes);
        const bestMemes = memesInstance.getBestMemes();
        expect(bestMemes[0]).to.equal(null);
        expect(bestMemes[1]).to.equal(null);
        expect(bestMemes[2]).to.equal(null);
    });
