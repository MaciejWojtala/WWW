(function() {
    ...
    if (json_data()) {
        ...
        function check_letter(arg : string) {
            if (arg.length !== 1)
                return false;
        ...
        function check_string(arg : string) : boolean {
            if (arg.length === 0)
                return false;
        ...
        function get_date() : Date {
            ...
            let month = (raw_date.getMonth() + 1).toString();
            let day = raw_date.getDate().toString();
            const year = raw_date.getFullYear().toString();
            ...
            const string_date = [year, month, day].join('-');
        ...
        function handler_2() : void {
            ...
            for (let i = 0; i < all.length; i++) {
                const element = all[i] as HTMLLIElement;
                element.addEventListener('click', (event) => {
                    ...
                    if (!form_grid.contains(element)) {
                        ...
                        element.style.backgroundColor = colors[it];
