(function () {
    ...
    class Statistic_Object {
        ...
        constructor(statistics: number[]) {
            this.statistics = new Array<number>(statistics.length);
            for (let i = 0; i < statistics.length; i++)
    ...
    function set_text_content_to_question_number(local_question_number : number) : void {
        ...
        const scoring_text : string =
                "Błędna odpowiedź na to pytanie odpowiada doliczeniu do czasu ";
    ...
    function fill_highscore_table() {
        ...
        let transaction = database.transaction([result_storage_name], "readonly");
        ...
        const request = objectStore.getAll() as IDBRequest<Result_Object[]>;
        request.onsuccess = function(event) {
            let proj = event.target as IDBRequest<Result_Object[]>;
            ...
            for (let i = 0; i < highscore_table_length; i++) {
                const row = document.getElementById
                        ...
                        (i+1).toString()) as HTMLElement;
                let cell = getCell(row, id_prefix + "2");
                if (i < result.length)
    ...
    function open_database(local_database_name : string,
            local_storage_1_name: string, local_storage_2_name: string) : void {
        let request = indexedDB.open(local_database_name);
        request.onerror = function (event) {
        ...
        request.onupgradeneeded = function (event) {
            ...
            local_database.createObjectStore
                    (local_storage_1_name);
            local_database.createObjectStore
                    (local_storage_2_name);
        }
    ...
    function delete_database(local_database_name : string) : void {
        database.close();
        let request = indexedDB.deleteDatabase(local_database_name);
    ...
    function save_data(object : Object, storage_name : string) : void {
        let transaction = database.transaction([storage_name], "readwrite");
        ...
        req.onsuccess = function(event) {
            let proj = event.target as IDBRequest;
            console.log(proj.result);
        }
    ...
    function save_result() : void {
        const object = new Result_Object(result_value);
        save_data(object, result_storage_name);
    }

    function save_statistics() : void {
        const object = new Statistic_Object(statistics);
