(function () {
    var start_button = document.getElementById("nowy_quiz");
    var highscore_button = document.getElementById("najlepsze_wyniki");
    var previous_question_button = document.getElementById("poprzednie_pytanie");
    var next_question_button = document.getElementById("następne_pytanie");
    var cancel_quiz_button = document.getElementById("anuluj_quiz");
    var end_quiz_button = document.getElementById("zakończ_quiz");
    ...
    var result_storage_name = "Result_storage";
    var statistics_storage_name = "Statistics_storage";
    var highscore_table_length = 5;
    ...
    var answers = new Array(quiz.questions.length);
    var statistics = new Array(quiz.questions.length);
    var spent_time = new Array(quiz.questions.length);
    var result_value = -1;
    var database;
    var time = -1;
    ...
    /*for (let i = 0; i < statistics.length; i++)
        statistics[i] = -1;*/
    var Result_Object = /** @class */ (function () {
        function Result_Object(result) {
            this.result = result;
        }
        return Result_Object;
    }());
    var Statistic_Object = /** @class */ (function () {
        function Statistic_Object(statistics) {
            this.statistics = new Array(statistics.length);
    ...
    function set_text_content_to_question_number(local_question_number) {
        ...
        var scoring_text = "Błędna odpowiedź na to pytanie odpowiada doliczeniu do czasu ";
        if (local_question_number == 1)
    ...
    function fill_highscore_table() {
        ...
        for (var i = 0; i < highscore_table_length; i++) {
        ...
        var transaction = database.transaction([result_storage_name], "readonly");
        transaction.oncomplete = function (event) { };
        transaction.onerror = function (event) {
            console.log("Nie udało się wczytać wyników");
        };
        var objectStore = transaction.objectStore(result_storage_name);
        var request = objectStore.getAll();
        request.onsuccess = function (event) {
            var proj = event.target;
    ...
    function open_database(local_database_name, local_storage_1_name, local_storage_2_name) {
        var request = indexedDB.open(local_database_name);
        request.onerror = function (event) {
            console.log("Otworzenie bazy danych nie powiodło się");
        };
        request.onsuccess = function (event) {
            console.log("Otwarto bazę 1");
            database = request.result;
            console.log("Otwarto bazę 2");
        ...
        request.onupgradeneeded = function (event) {
            ...
            local_database.createObjectStore(local_storage_1_name);
            local_database.createObjectStore(local_storage_2_name);
        };
        request.onblocked = function (event) {
            console.log("Baza zablokowana");
        };
    }
    /// Funkcja ułatwiająca testowanie
    function delete_database(local_database_name) {
        database.close();
        var request = indexedDB.deleteDatabase(local_database_name);
        request.onsuccess = function () {
            console.log("Usunięto bazę");
        };
        request.onerror = function () {
            console.log("Nie można usunąć bazy");
        };
    ...
    function save_data(object, storage_name) {
        ...
        transaction.oncomplete = function (event) { };
        transaction.onerror = function (event) {
            console.error("Błąd zapisu wyniku");
        };
        ...
        req.onerror = function (event) {
            var proj = event.target;
            console.log(proj.error);
        };
    }
    function save_result() {
        var object = new Result_Object(result_value);
        save_data(object, result_storage_name);
    }
    function save_statistics() {
        var object = new Statistic_Object(statistics);
    ...
    function get_polish_suffix(value) {
        ...
        if (remainder <= 1 || remainder >= 5 || (value >= 12 && value <= 14))
            return "";
        return "y";
    }
    ...
    window.addEventListener('load', function (event) {
        console.log("load");
        open_database(database_name, result_storage_name, statistics_storage_name);
    });
    start_button.addEventListener('click', function (event) {
        ...
        for (var i = 0; i < spent_time.length; i++)
            spent_time[i] = 0;
        question_number = 1;
        time = 0;
    ...
    previous_question_button.addEventListener('click', function (event) {
    ...
    highscore_button.addEventListener('click', function (event) {
        var header = document.getElementById("tytuł_strony");
    ...
    end_quiz_button.addEventListener('click', function (event) {
        var header = document.getElementById("tytuł_strony");
        var grid_result = document.getElementById("grid_liczba_punktów");
